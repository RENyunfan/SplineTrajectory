...
void runEvaluationTests()
{
    ...
    // Setup MinSnap trajectory
    snapOpt.reset(iS, fS, route.cols() - 1);
    snapOpt.generate(route.block(0, 1, 3, test_segments - 1), ts);
    ...
    boundary.start_jerk.setZero();
    boundary.end_velocity.setZero();
    boundary.end_acceleration.setZero();
    boundary.end_jerk.setZero();
    septicSpline.update(time_segments, spline_points, 0.0, boundary);
    
    double total_duration = minSnapTraj.getTotalDuration();
    ...
    // MinSnap random evaluation - Position
    t0 = std::chrono::high_resolution_clock::now();
    std::vector<Vector3d> ms_pos_random(num_random_points);
    for (int i = 0; i < num_random_points; ++i)
    {
        ms_pos_random[i] = minSnapTraj.getPos(random_times[i]);
    }
    t1 = std::chrono::high_resolution_clock::now();
    double ms_random_pos_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    ...
    // MinSnap random evaluation - Velocity
    t0 = std::chrono::high_resolution_clock::now();
    std::vector<Vector3d> ms_vel_random(num_random_points);
    for (int i = 0; i < num_random_points; ++i)
    {
        ms_vel_random[i] = minSnapTraj.getVel(random_times[i]);
    }
    t1 = std::chrono::high_resolution_clock::now();
    double ms_random_vel_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    ...
    // MinSnap random evaluation - Acceleration
    t0 = std::chrono::high_resolution_clock::now();
    std::vector<Vector3d> ms_acc_random(num_random_points);
    for (int i = 0; i < num_random_points; ++i)
    {
        ms_acc_random[i] = minSnapTraj.getAcc(random_times[i]);
    }
    t1 = std::chrono::high_resolution_clock::now();
    double ms_random_acc_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    ...
    std::cout << "Evaluation Type\tMinSnap(μs)\tSepticSpline(μs)\tRatio(MS/SS)" << std::endl;
    std::cout << "Position\t" << std::fixed << std::setprecision(1) 
              << ms_random_pos_time << "\t\t" << ss_random_pos_time << "\t\t\t" 
              << std::setprecision(3) << (ms_random_pos_time / ss_random_pos_time) << std::endl;
    std::cout << "Velocity\t" << std::fixed << std::setprecision(1) 
              << ms_random_vel_time << "\t\t" << ss_random_vel_time << "\t\t\t" 
              << std::setprecision(3) << (ms_random_vel_time / ss_random_vel_time) << std::endl;
    std::cout << "Acceleration\t" << std::fixed << std::setprecision(1) 
              << ms_random_acc_time << "\t\t" << ss_random_acc_time << "\t\t\t" 
              << std::setprecision(3) << (ms_random_acc_time / ss_random_acc_time) << std::endl;
    ...
    t0 = std::chrono::high_resolution_clock::now();
    std::vector<Vector3d> ms_pos_batch(uniform_times.size());
    for (size_t i = 0; i < uniform_times.size(); ++i)
    {
        ms_pos_batch[i] = minSnapTraj.getPos(uniform_times[i]);
    }
    t1 = std::chrono::high_resolution_clock::now();
    double ms_batch_pos_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    ...
    t0 = std::chrono::high_resolution_clock::now();
    std::vector<Vector3d> ms_vel_batch(uniform_times.size());
    for (size_t i = 0; i < uniform_times.size(); ++i)
    {
        ms_vel_batch[i] = minSnapTraj.getVel(uniform_times[i]);
    }
    t1 = std::chrono::high_resolution_clock::now();
    double ms_batch_vel_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    ...
    for (size_t i = 0; i < uniform_times.size(); ++i)
    {
        ms_acc_batch[i] = minSnapTraj.getAcc(uniform_times[i]);
    }
    t1 = std::chrono::high_resolution_clock::now();
    double ms_batch_acc_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    ...
    std::vector<Vector3d> ms_pos_consistency(num_consistency_points);
    std::vector<Vector3d> ms_vel_consistency(num_consistency_points);
    std::vector<Vector3d> ms_acc_consistency(num_consistency_points);
    
    for (int i = 0; i < num_consistency_points; ++i)
    {
        ms_pos_consistency[i] = minSnapTraj.getPos(consistency_times[i]);
        ms_vel_consistency[i] = minSnapTraj.getVel(consistency_times[i]);
    ...
    double pos_max_error = calculateMaxError(ms_pos_consistency, ss_pos_consistency_vec);
    double vel_max_error = calculateMaxError(ms_vel_consistency, ss_vel_consistency_vec);
    double acc_max_error = calculateMaxError(ms_acc_consistency, ss_acc_consistency_vec);
...
int main()
{
    ...
    double minSnapTime_us, septicSplineTime_us;
    int groupSize = 1000;
    ...
    std::cout << "Performance Comparison: MinSnap vs SepticSpline (3D)" << std::endl;
    std::cout << "===============================================" << std::endl;
    std::cout << "Segments\tMinSnap(μs)\tSepticSpline(μs)\tRatio(Septic/MinSnap)" << std::endl;

    for (int segments : test_segments)
    {
        minSnapTime_us = septicSplineTime_us = 0.0;
        ...
        for (int j = 0; j < groupSize; j++)
        {
            ...
            snapOpt.reset(iS, fS, route.cols() - 1);
            snapOpt.generate(route.block(0, 1, 3, segments - 1), ts);
            snapOpt.getTraj(minSnapTraj);
            ...
            // 测试 SepticSpline - 直接使用时间段构造
            tc1 = std::chrono::high_resolution_clock::now();
            septicSpline.update(time_segments, spline_points, 0.0, boundary);
            tc2 = std::chrono::high_resolution_clock::now();
            
            septicSplineTime_us += std::chrono::duration_cast<std::chrono::microseconds>(tc2 - tc1).count();
        }

        double avgMinSnapTime = minSnapTime_us / groupSize;
        double avgSepticTime = septicSplineTime_us / groupSize;
        double ratio = avgSepticTime / avgMinSnapTime;

        std::cout << segments << "\t\t" 
                  << std::fixed << std::setprecision(2) << avgMinSnapTime << "\t\t" 
                  << avgSepticTime << "\t\t\t" 
                  << std::setprecision(3) << ratio << std::endl;
    }

    std::cout << "\nConstruction test completed successfully!" << std::endl;
    std::cout << "Note: Lower time is better. Ratio > 1 means MinSnap is faster, ratio < 1 means SepticSpline is faster." << std::endl;
