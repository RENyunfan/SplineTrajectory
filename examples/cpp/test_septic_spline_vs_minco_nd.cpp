...
struct BatchPerformanceResult
{
    ...
    double septic_batch_evaluation_us;
...
void printBatchPerformance(const BatchPerformanceResult &r, const std::string &name)
{
    ...
    std::cout << "  - SepticSpline 批量评估: " << r.septic_batch_evaluation_us << " μs\n";
    std::cout << "  - MINCO 批量评估: " << r.minco_batch_evaluation_us << " μs\n";
    std::cout << "  - 总查询点数: " << r.total_points << "\n";
    std::cout << "  - 性能提升比 (MINCO/SepticSpline): " << r.speedup_ratio << "x\n";
}

// 一致性测试（包含snap/四阶导数）
template <typename VecType>
ConsistencyResult testConsistency(
    const SplineVector<VecType> &septic_pos, const SplineVector<VecType> &septic_vel,
    const SplineVector<VecType> &septic_acc, const SplineVector<VecType> &septic_jerk,
    const SplineVector<VecType> &septic_snap,
    ...
    const SplineVector<VecType> &minco_snap)
{
    ...
    int N = septic_pos.size();
    if (N == 0)
        return result;

    for (int i = 0; i < N; ++i)
    {
        double pos_diff = (septic_pos[i] - minco_pos[i]).norm();
        double vel_diff = (septic_vel[i] - minco_vel[i]).norm();
        double acc_diff = (septic_acc[i] - minco_acc[i]).norm();
        double jerk_diff = (septic_jerk[i] - minco_jerk[i]).norm();
        double snap_diff = (septic_snap[i] - minco_snap[i]).norm();
...
void runSepticTest()
{
    ...
    auto start_jerk_eigen = generateRandomVector(DIM, 3.0);
    auto end_jerk_eigen = generateRandomVector(DIM, 3.0);

    using SepticSpline = SplineTrajectory::SepticSplineND<DIM>;
    using VectorD = typename SepticSpline::VectorType;

    typename SepticSpline::MatrixType waypoints = waypoints_mat;
    ...
    // 七次样条拟合性能测试
    double total_septic_fit_time_us = 0;
    for (int i = 0; i < fit_runs; ++i)
    {
        auto t1 = std::chrono::high_resolution_clock::now();
        SepticSpline septic_spline(time_segments_vec, waypoints, start_time, bc);
        auto t2 = std::chrono::high_resolution_clock::now();
        total_septic_fit_time_us += std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
    }
    std::cout << "  - SepticSpline" << DIM << "D 拟合: " << std::fixed << std::setprecision(2)
              << total_septic_fit_time_us / fit_runs << " μs" << std::endl;
    ...
    {
        auto t1 = std::chrono::high_resolution_clock::now();
        std::vector<minco::MINCO_S4NU> minco_solvers(num_minco_instances);
        for (int j = 0; j < num_minco_instances; ++j)
        {
            int start_dim = j * 3;

            // 使用安全的数据填充方法
            Eigen::MatrixXd head = Eigen::MatrixXd::Zero(3, 4), tail = Eigen::MatrixXd::Zero(3, 4);
            ...
            Eigen::Vector3d head_jerk = make3DFromND(start_jerk_eigen, start_dim);
            ...
            Eigen::Vector3d tail_jerk = make3DFromND(end_jerk_eigen, start_dim);

            head.col(0) = head_pos;
            head.col(1) = head_vel;
            head.col(2) = head_acc;
            head.col(3) = head_jerk;
            tail.col(0) = tail_pos;
            tail.col(1) = tail_vel;
            tail.col(2) = tail_acc;
            tail.col(3) = tail_jerk;
    ...
    std::cout << "  - MINCO_S4NU 模拟 " << DIM << "D 拟合: " << std::fixed << std::setprecision(2)
    ...
    // 创建用于查询的MINCO实例
    std::vector<minco::MINCO_S4NU> minco_solvers_for_query(num_minco_instances);
    std::vector<Trajectory<7>> minco_trajs(num_minco_instances); // 七次样条使用Trajectory<7>
    ...
    double dt = 0.01;
    auto time_seq = septic_spline.getTrajectory().generateTimeSequence(dt);
    int num_queries = time_seq.size();

    std::cout << "\n2.2) 单点查询性能 (" << num_queries << "个点):" << std::endl;

    // 七次样条单点查询性能
    std::function<void(int)> septic_func = [&](int i)
    { septic_spline.getTrajectory().evaluate(time_seq[i]); };
    auto perf_septic = measurePerformance(septic_func, num_queries);
    printPerformance(perf_septic, "  - SepticSpline" + std::to_string(DIM) + "D");
    ...
    printPerformance(perf_minco, "  - MINCO_S4NU 模拟 " + std::to_string(DIM) + "D");

    std::cout << "\n2.3) 普通批量查询性能 (一次性查询 " << num_queries << " 个点):" << std::endl;

    // 七次样条批量查询
    auto t1 = std::chrono::high_resolution_clock::now();
    auto septic_results_position = septic_spline.getTrajectory().evaluate(time_seq, Deriv::Pos);
    auto septic_results_velocity = septic_spline.getTrajectory().evaluate(time_seq, Deriv::Vel);
    auto septic_results_acceleration = septic_spline.getTrajectory().evaluate(time_seq, Deriv::Acc);
    auto septic_results_jerk = septic_spline.getTrajectory().evaluate(time_seq, Deriv::Jerk);
    auto septic_results_snap = septic_spline.getTrajectory().evaluate(time_seq, Deriv::Snap);
    auto t2 = std::chrono::high_resolution_clock::now();
    double septic_batch_us = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
    std::cout << "  - SepticSpline" << DIM << "D 普通批量查询: " << septic_batch_us << " μs" << std::endl;
    ...
    for (size_t i = 0; i < time_seq.size(); ++i)
    {
        ...
        for (int j = 0; j < num_minco_instances; ++j)
        {
            ...
            if (start_dim < DIM)
            {
                ...
                Eigen::Vector3d snap_3d = minco_trajs[j].getSnap(t); // 七次样条的四阶导数
    ...
    std::cout << "  - MINCO_S4NU 普通批量查询: " << minco_batch_us << " μs" << std::endl;
    if (septic_batch_us > 0)
        std::cout << "  - 性能比 (MINCO/SepticSpline): " << std::fixed << std::setprecision(3)
                  << minco_batch_us / septic_batch_us << "x" << std::endl;

    std::cout << "\n2.4) 普通批量查询一致性检查:" << std::endl;
    auto cons = testConsistency(septic_results_position, septic_results_velocity,
                                septic_results_acceleration, septic_results_jerk, septic_results_snap,
                                minco_pos, minco_vel, minco_acc, minco_jerk, minco_snap);
    printConsistency(cons);

    std::cout << "\n2.5) PPolyND::derivative() 方法验证 (一阶导数):" << std::endl;
    auto ppoly = septic_spline.getPPoly();
    ...
    if (!time_seq.empty())
    {
        for (size_t i = 0; i < time_seq.size(); ++i)
        {
            double diff = (septic_results_velocity[i] - vel_from_d_ppoly[i]).norm();
    ...
    if constexpr (DIM == 3)
    {
        ...
        // 获取 SepticSplineND 的能量
        double septic_energy = septic_spline.getEnergy();
        
        // 获取 MINCO_S4NU 的能量 (只有第一个实例，因为DIM=3时只有一个MINCO实例)
        double minco_energy = 0.0;
        minco_solvers_for_query[0].getEnergy(minco_energy);
        
        // 计算差异
        double energy_diff = std::abs(septic_energy - minco_energy);
        double relative_error = (septic_energy != 0.0) ? (energy_diff / std::abs(septic_energy)) : 0.0;
        
        std::cout << std::fixed << std::setprecision(6);
        std::cout << "  - SepticSplineND 能量: " << septic_energy << std::endl;
        std::cout << "  - MINCO_S4NU 能量: " << minco_energy << std::endl;
...
int main()
{
    std::cout << "\n===== 多维 SepticSplineND vs MINCO_S4NU 性能与一致性对比 =====\n";
    std::cout << "SepticSplineND 使用块三对角矩阵与追赶法求解，使用 PPolyND 进行高效批量查询，\n";
    std::cout << "MINCO是常用的最小控制量轨迹库，本质是夹持多项式样条 (s=4，即七次夹持样条)\n";
    std::cout << "这里通过组合多个3D实例来模拟高维七次样条\n";
